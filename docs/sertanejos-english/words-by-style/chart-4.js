...
// Now that it's all said and done, I'll add some labels and titles
    ...
    highlight.append("text")
      .text("Each circle is a word. All words are represented here, apart from 'love',")
      .attr("class","chart-title")
      .attr("x", width/2)
      .attr("y",height * 0.1)
      .attr("dy", 30)
    ...
    highlight.append("text")
      .text("since it's usage is so ubiquitous that it would break the chart's scale")
      .attr("class","chart-title")
      .attr("x", width/2)
      .attr("y",height * 0.1)
      .attr("dy", 50)
    ...
    highlight.append("text")
      .text("word is common in both genres ↑")
      .attr("class","chart-title")
      .attr("x",width/2)
      .attr("y",height * 0.35)
      .attr("text-anchor","middle")
    ...
    highlight.append("text")
      .text("word is more common in traditional sertanejo ⟵ ")
      .attr("class","chart-title")
      .attr("x",width/2 - 30)
      .attr("y",height)
      .attr("dy", 30)
    ...
    highlight.append("text")
      .text("⟶ words is more common in university sertanejo")
      .attr("class","chart-title")
      .attr("x", width/ 2 + 30)
      .attr("y", height)
      .attr("dy", 30)
          ...
          d3.select('#tooltip')
            ...
            .html(function(){
              if ( node.attr("mais-comum-antigo") > (node.attr("mais-comum-novo")) ) {
                return '<h4><b>' + node.attr("word") + '</b></h4>' + 'Appearences in each 10k words:<b> ' + node.attr("ratio-geral") + 
              '</b><br><b>' + node.attr("mais-comum-antigo") + 'x</b> more common in traditional sertanejo';
              }
              else {
              return '<h4><b>' + node.attr("word") + '</b></h4>' + 'Appearences in each 10k words:<b> ' + node.attr("ratio-geral") + 
              '</b><br><b>' + node.attr("mais-comum-novo") + 'x</b> more common in university sertanejo';
              }
            })
