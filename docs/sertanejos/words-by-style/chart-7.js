...
// An svg that lies behind the canvas
  var svgHolder = d3.select("#chart-7")
    .append("svg")
...
// To highlight only the words I want
  var filteredDatapoints = datapoints.filter(function(d){
    if (d.index === "caboclo" || d.index === "fazenda" || d.index === "peão" ||
      d.index === "lavrador" || d.index === "sertão"){
      return d
    }
  })
...
// Adding title and labels to the svg behind the canvas
    ...
    svgHolder.append("text")
      .text("O sertanejo tradicional se preocupava com o campo. O universitário, nem tanto")
      .attr("class","chart-title")
      .attr("x", width/2)
      .attr("y",height * 0.1)
      .attr("dy", 30)
