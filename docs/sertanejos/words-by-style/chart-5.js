...
// An svg that lies behind the canvas
  var svgHolder = d3.select("#chart-5")
    .append("svg")
...
// To highlight only the words I want
  var filteredDatapoints = datapoints.filter(function(d){
    if (d.index === "balada" || d.index === "bar" || d.index === "cerveja" ||
      d.index === "cachaça") {
      return d
    }
  })
...
// Adding title and labels to the svg behind the canvas
    ...
    svgHolder.append("text")
      .text("Enquanto os sertanejos das antigas bebem cachaça,")
      .attr("class","chart-title")
      .attr("x", width/2)
      .attr("y",height * 0.1)
      .attr("dy", 30)
    ...
    svgHolder.append("text")
      .text("a nova geração pede cerveja - de preferência, na balada")
      .attr("class","chart-title")
      .attr("x", width/2)
      .attr("y",height * 0.1)
      .attr("dy", 50)
    ...
    svgHolder.append("text")
      .text("comum em ambos os gêneros ↑")
      .attr("class","chart-title")
      .attr("x",width/2)
      .attr("y",height * 0.35)
      .attr("text-anchor","middle")
